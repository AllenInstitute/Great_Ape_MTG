...
low_reads <- c("CTNND1", "DISC1", "ATP6V0C", "PPP3R1", "PPP2CA", "RAB5C", "RPL26", "VAMP2", "BLOC1S6")
fold.change <- fold.change[!(rownames(fold.change) %in% low_reads), ] 
# Inf values:
fold.change$h_fc[fold.change$h_fc == "-Inf"] <- -4.563528 
fold.change$c_fc[fold.change$c_fc == "-Inf"] <- -5.09407825

# plot heatmap:
heatmap<-fold.change[,4:6]
heatmap<-heatmap[order(-heatmap$h_fc),] 
heatmap<-data.matrix(heatmap)
break_heatmap <-c(seq(-5.4,5.4,by=0.2))
color_heatmap <-colorRampPalette(c("grey25","white","#4876ff"))(54)
