...
# chimp vs gorilla:
c_g <- read.table(here("data/astrocytes", "Astro_chimp_vs_gorilla_sig_genes.csv"), sep=",", header=TRUE)
c_g <- c_g[c_g$padj<0.01 & (c_g$log2FoldChange<(-0.5) | c_g$log2FoldChange>(0.5)),] #2173 genes
CG<-c_g[(1)]
CG$CG<-1

# human vs rhesus:
h_r <- read.table(here("data/astrocytes", "Astro_human_vs_rhesus_sig_genes.csv"), sep=",", header=TRUE)
h_r <- h_r[h_r$padj<0.01 & (h_r$log2FoldChange<(-0.5) | h_r$log2FoldChange>(0.5)),]
HR<-h_r[(1)]
HR$HR<-1

# chimp vs rhesus:
c_r <- read.table(here("data/astrocytes", "Astro_chimp_vs_rhesus_sig_genes.csv"), sep=",", header=TRUE)
c_r <- c_r[c_r$padj<0.01 & (c_r$log2FoldChange<(-0.5) | c_r$log2FoldChange>(0.5)),]
CR<-c_r[(1)]
CR$CR<-1

# gorilla vs rhesus:
g_r <- read.table(here("data/astrocytes", "Astro_gorilla_vs_rhesus_sig_genes.csv"), sep=",", header=TRUE)
g_r <- g_r[g_r$padj<0.01 & (g_r$log2FoldChange<(-0.5) | g_r$log2FoldChange>(0.5)),]
GR<-g_r[(1)]
GR$GR<-1

# human vs marmoset:
h_m <- read.table(here("data/astrocytes", "Astro_human_vs_marmoset_sig_genes.csv"), sep=",", header=TRUE)
h_m <- h_m[h_m$padj<0.01 & (h_m$log2FoldChange<(-0.5) | h_m$log2FoldChange>(0.5)),]
HM<-h_m[(1)]
HM$HM<-1

# chimp vs marmoset:
c_m <- read.table(here("data/astrocytes", "Astro_chimp_vs_marmoset_sig_genes.csv"), sep=",", header=TRUE)
c_m <- c_m[c_m$padj<0.01 & (c_m$log2FoldChange<(-0.5) | c_m$log2FoldChange>(0.5)),]
CM<-c_m[(1)]
CM$CM<-1

# gorilla vs marmoset:
g_m <- read.table(here("data/astrocytes", "Astro_gorilla_vs_marmoset_sig_genes.csv"), sep=",", header=TRUE)
g_m <- g_m[g_m$padj<0.01 & (g_m$log2FoldChange<(-0.5) | g_m$log2FoldChange>(0.5)),]
GM<-g_m[(1)]
GM$GM<-1

# rhesus vs marmoset:
r_m <- read.table(here("data/astrocytes", "Astro_rhesus_vs_marmoset_sig_genes.csv"), sep=",", header=TRUE)
...
ggsave("astro_upset_fig_s5a.pdf")
